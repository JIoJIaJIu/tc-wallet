...
export default class StatsFetcher {
  ...
  tokens() {
    return this.balance ? this.balance : { TT: 0 };
  }

  totalTokens() {
    const totalStaked = this.totalStaked();
    const totalBeingUnstaked = this.totalBeingUnstaked();
    const totalTokens = this.tokens().TT || new Decimal(0);
  ...
  delegatedStats() {
    ...
    return {
      cpuWeight: `${totalCpuAmount.minus(selfCpuAmount).toFixed(4)} TT`,
      netWeight: `${totalNetAmount.minus(selfNetAmount).toFixed(4)} TT`,
      totalStaked: this.totalStaked()
    };
  }
}
